...
<head>
    ...
    <title>{{__('messages.es_106')}}</title>
...
<body class="flex flex-col min-h-screen overflow-x-hidden">
    ...
    <main class="flex-grow">
        <div class="w-full h-auto flex flex-col md:items-start items-center p-4 gap-2 lg:px-16">
            <h2 class="font-bold text-xl text-vh-green lg:font-bold lg:text-3xl">{{__('messages.es_106')}}</h2>
        </div>
            ...
            <div class="bg-cover w-full flex justify-center items-center">
                <div class="w-full bg-white lg:p-5 p-2 bg-opacity-40">
                    <div class="w-12/12 mx-auto rounded-2xl bg-white lg:p-5 p-2">
                        <div>
                            @if ($recetas->isEmpty())
                                <p class="text-center text-gray-500">{{__('messages.es_107')}}</p>
                            @else
                                    ...
                                    @foreach ($recetas as $receta)
                                        <article
                                            ...
                                            <div class="p-6 flex flex-col h-full">
                                                ...
                                                <p class="text-gray-600 mb-2">{{__('messages.es_108')}}
                                                    {{ $receta->hora_entrega }}
                                                </p>
                                                <p class="text-gray-600 mb-2">{{__('messages.es_109')}} {{ $receta->estado }}
                                                </p>
                                                <h4 class="font-medium text-lg leading-8 mb-2">{{__('messages.es_110')}}</h4>
